...
const { processCrawl } = require("./processCrawl");
const { processSingleFile } = require("./processSingleFile");
const { processLink, getLinkText } = require("./processLink");
const { processLinkWithFirecrawl } = require("./processLinkWithFirecrawl");
...
app.post(
  ...
  async function (request, response) {
    const { link, limit } = reqBody(request);
    try {
      const { success, reason, documents = [] } = await processCrawl(link, limit);
  ...
  async function (request, response) {
    ...
    try {
      const {
        ...
        reason,
        documents = [],
      } = await processLinkWithFirecrawl(link);
      response.status(200).json({ url: link, success, reason, documents });
    } catch (e) {
      console.error(e);
      response.status(200).json({
